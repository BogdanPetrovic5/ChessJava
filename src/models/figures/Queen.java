...
public class Queen extends Figure{
    public Queen(Figures figureType, String color, String url, int x, int y, IField field, IGameController gameController) {
    ...
    public Figure canMove(int x, int y, boolean isSelected) {
        ...
        if(!isSelected){
            for(int i = this.getY() + 1; i < fields.length;i++){
                if(fields[i][this.getX()].getFigure() == null){
                    fields[i][this.getX()].setBorder(BorderFactory.createLineBorder(Color.YELLOW, 3));
                    addAvailableField(fields[i][this.getX()]);
                }else {
                    if(!fields[i][this.getX()].getFigure().getColor().equals(this.getColor())) {
                        fields[i][this.getX()].setBorder(BorderFactory.createLineBorder(Color.RED, 3));
                        addAvailableField(fields[i][this.getX()]);
                    }
                    break;
                }
            ...
            for(int i = this.getX() + 1; i < fields.length; i++){
                if(fields[this.getY()][i].getFigure() == null){
                    fields[this.getY()][i].setBorder(BorderFactory.createLineBorder(Color.YELLOW, 3));
                    addAvailableField(fields[this.getY()][i]);
                }else {
                    if(!fields[this.getY()][i].getFigure().getColor().equals(this.getColor())){
                        fields[this.getY()][i].setBorder(BorderFactory.createLineBorder(Color.RED, 3));
                        addAvailableField(fields[this.getY()][i]);
                    }
                    break;
                }
